...
class DirectAccessTestUtils {
    ...
    fun createCredentialData(
        context: Context,
        authKeyCert: MDocDocument.MDocSigningKeyCertificationRequest,
        docType: String, readerPublicKeys: ArrayList<Certificate>?
    ): ByteArray {
        return try {
            val issuerKeypair = generateIssuingAuthorityKeyPair()
            DocumentDataParser.generateCredentialData(
                docType,
    ...
    fun validateMdocResponse(response: DeviceResponseParser.DeviceResponse, entries: Array<String?>): Boolean {
        ...
        for (doc in documentList) {
            for (eleId in entries) {
                val eleVal = doc.getIssuerEntryData(DocumentDataParser.MDL_NAMESPACE, eleId!!)
                    ?: return false
            }
        }
        return true
